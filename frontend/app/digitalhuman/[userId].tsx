...
export default function DigitalHumanChat() {
  ...
  const target = useQuery(api.auth.me, targetUserId ? { userId: targetUserId } : 'skip');
  ...
  if (!auth || auth.status !== 'authenticated' || !targetUserId) return null;
  ...
  async function handleSend() {
    ...
    setMessages((prev) => [
      ...prev,
      { id: `me-${Date.now()}`, from: 'me', text: userMsg, ts: Date.now() },
    ]);
    setSending(true);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `twin-${Date.now()}`, from: 'twin', text: res.reply, ts: Date.now() },
      ]);
    } catch (e: unknown) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerMeta}>
          <Text style={styles.headerName}>{target?.nickname ?? '...'}</Text>
          <Text style={styles.headerMode}>· {MODE_LABEL[mode]}</Text>
        </View>
      ...
      >
        ...
        >
          {messages.length === 0 && (
            <Text style={styles.placeholder}>
              和 {target?.nickname ?? '...'} 的 AI 分身打个招呼吧～
            </Text>
          )}
          {messages.map((m) => (
            <View
              key={m.id}
              style={[styles.bubbleWrap, m.from === 'me' ? styles.bubbleMe : styles.bubbleTwin]}
            >
              <StickerCard
        ...
        <View style={styles.inputBar}>
          ...
          >
            <HandwrittenTitle size={16} color="#1F1F1F">发送</HandwrittenTitle>
          </PressableScale>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  headerMeta: { flex: 1, flexDirection: 'row', alignItems: 'baseline', gap: 6 },
  headerName: { fontSize: 16, fontWeight: '800', color: Colors.text },
  headerMode: { fontSize: 12, color: Colors.textSecondary },
  aiTag: {
  ...
  aiTagText: { fontSize: 11, fontWeight: '800', color: '#1F1F1F' },
  messages: { padding: Spacing.md, gap: Spacing.sm, paddingBottom: Spacing.lg },
  placeholder: { textAlign: 'center', color: Colors.textMuted, marginTop: Spacing.xl },
  bubbleWrap: { maxWidth: '80%', gap: 4 },
  bubbleMe: { alignSelf: 'flex-end' },
  bubbleTwin: { alignSelf: 'flex-start' },
  bubble: { paddingHorizontal: 14, paddingVertical: 10 },
  bubbleText: { fontSize: 15, color: Colors.text, lineHeight: 22 },
  disclaimer: { fontSize: 11, color: Colors.textMuted },
  disclaimerLink: { color: Colors.romance, fontWeight: '700', textDecorationLine: 'underline' },
  errorText: { color: Colors.error, fontSize: 13, padding: Spacing.md, textAlign: 'center' },
  inputBar: {
    flexDirection: 'row',
    alignItems: 'flex-end',
    padding: Spacing.md,
